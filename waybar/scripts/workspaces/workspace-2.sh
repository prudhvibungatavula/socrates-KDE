...
uuid=$(qdbus6 --literal org.kde.KWin /VirtualDesktopManager org.freedesktop.DBus.Properties.Get org.kde.KWin.VirtualDesktopManager desktops 2>/dev/null | grep -oP '"[a-f0-9-]{36}"' | sed -n '2p' | tr -d '"')
